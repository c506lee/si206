# Your name: Eugene Lee 
# Your student id: leeyooj
# Your email: leeyooj@umich.edu
...
class Animal_Fact_Generator:
	...
	def __str__(self):
		newstring = ",".join(self.animal_list)

		return str(newstring)
	
	def random_fact(self):
		#need to use random to get the INTERVAl, turn that into a variable
		randfact = random.randint(0, len(self.fact_list)-1)
		self.fact_history_list.append(randfact)


		return self.fact_list[randfact] + " - " + self.animal_list[randfact]


	def get_fact_for_animal(self, name):

		if name in self.animal_list:

			animalindex = self.animal_list.index(name)

			return self.fact_list[animalindex]

		else:
			return "Sorry! I do not have any facts for " + name
